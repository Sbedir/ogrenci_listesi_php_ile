...
<div class="container">
  ...
  <table id='ogrTable' class="table table-striped table-bordered">
    ...
    while ($ogrencicek=$ogrencisor->fetch(PDO::FETCH_ASSOC)) {?>
      <tr>
        <td><img src="<?=$ogrencicek['ogr_resim']!=''?$ogrencicek['ogr_resim']:'images/resim-yok.jpg';?>" style="width:100px"/></td>
        <td><?=$ogrencicek['ad'];?></td>
        <td><?=$ogrencicek['soyad'];?></td>
        <td><?=$ogrencicek['okul_no'];?></td>
        <td><?=$ogrencicek['okul_ad'];?></td>
        <td><?=$ogrencicek['fakulte'];?></td>
        <td><?=$ogrencicek['bolum'];?></td>
        <td><?=$ogrencicek['adres'];?></td>
        <td><?=$ogrencicek['telefon'];?></td>
        <td><?=$ogrencicek['mail'];?></td>
        <td><?=$ogrencicek['okul_baslama_zaman'];?></td>
        <td><?=$ogrencicek['okul_bitis_zaman'];?></td>
        <td>
         <!--modal güncelleme-->
...
<script>
    ...
    function ogr_guncelle(obj)
    {
        ...
        $('#gid').val(obj.id);
        $('#gad').val(obj.ad);
        $('#gresimdel').val(obj.ogr_resim);
        $('#gsoyad').val(obj.soyad);
        $('#gokul_numarasi').val(obj.okul_no);
        $('#gokul_adi').val(obj.okul_ad);
        $('#gfakulte').val(obj.fakulte);
        $('#gbolum').val(obj.bolum);
        $('#gadres').val(obj.adres);
        $('#gtelefon').val(obj.telefon);
        $('#gmail').val(obj.mail);
        $('#gokul_baslangic_tarihi').val(obj.okul_baslama_zaman);
        $('#gokul_bitis_tarihi').val(obj.okul_bitis_zaman);
